...
import type { TirednessRequest, TirednessUpdate } from "@/app/got-houses-v2/types";
...
function climateHarshness(army: TirednessRequest["armies"][number]): "harsh" | "fair" | "kind" {
  const ground = army.holdGround.toLowerCase();
  const route = army.marchRoute?.route.toLowerCase() ?? "";
  const fit = (army.regionFit ?? "").toLowerCase();

  // The region trait states outright whether these men are bred for this
  // country, so trust it before falling back to reading terrain prose.
  if (fit.includes("bred for this country")) return "kind";
  if (fit.includes("strangers to this country")) return "harsh";

  const swampOrFen = /swamp|bog|fen|neck|causeway|fever/.test(`${ground} ${route}`);
  const isNorth = army.homeland.toLowerCase().includes("cold");
  if (swampOrFen && !isNorth) return "harsh";
  if (/excellent rest|superb rest|comfortable rest|good lodging|solid lodging/.test(ground)) {
    return "kind";
  }
  return "fair";
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey) {
      console.warn("[got-houses-v2/tiredness] ANTHROPIC_API_KEY not set — using fallback");
...
${activityLines}${mergeSection}`;
    ...
    console.log(`[got-houses-v2/tiredness] Adjudicating ${armies.length} armies`);
    ...
    console.log("[got-houses-v2/tiredness] Raw response length:", rawText.length);

    const jsonMatch = rawText.match(/\[[\s\S]*\]/);
    if (!jsonMatch) {
      console.error("[got-houses-v2/tiredness] No JSON array found. Using fallback.");
      return NextResponse.json(fallbackTiredness(armies));
    }
    ...
    try {
      ...
      console.error("[got-houses-v2/tiredness] JSON parse error:", msg);
      return NextResponse.json(fallbackTiredness(armies));
    }

    if (!Array.isArray(parsed) || parsed.length === 0) {
      console.error("[got-houses-v2/tiredness] Invalid response format. Using fallback.");
      return NextResponse.json(fallbackTiredness(armies));
    }
    ...
    console.log("[got-houses-v2/tiredness] Success — updated", updates.length, "armies");
    return NextResponse.json(updates);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[got-houses-v2/tiredness] Unexpected error:", msg);
    return NextResponse.json(
