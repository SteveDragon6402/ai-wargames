...
Use tools first if you need hold/map/army/event facts (inspect_my_castle, survey_map, find_forces, search_faction_events, get_battle_logs). Then greet them in your own voice.`,
      ctx,
      maxRounds: 4,
      maxTokens: 500,
    });
    ...
    if (!speech) {
      return NextResponse.json(
        {
          error: `${target.name} gave no answer — the words never came back. Try again.`,
        },
        { status: 500 }
      );
    }
