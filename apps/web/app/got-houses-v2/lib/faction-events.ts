...
export function adviceVsActionHints(
  ...
  for (const a of recentAdvice) {
    ...
    for (const m of marches) {
      ...
      // Soft heuristic: southward warnings vs southward place names
      const warnedSouth =
        /south|crownlands|neck|twins|riverrun|kingsroad/.test(adviceLower);
      const wentThatWay =
        /south|twins|riverrun|harrenhal|king|maidenpool|moat/.test(marchLower);
