...
function GarrisonPanelInner({
  ...
  // Mirrors the reducer: a seat still held by its own house keeps its native
  // default; a conquered one keeps whatever its capture pledge demanded.
  const openPledge = (state.capturePledges ?? []).find(
    (p) => p.holdId === holdId && p.faction === faction
  );
  const floor =
    mode === "withdraw" && isFriendlyTo(hs, faction)
      ? hs.controller === hs.homeFaction
        ? seed.defaultGarrison
        : openPledge?.minimumMen ?? minimumHoldingGarrison(holdId, currentMen)
      : 0;
  const maxWithdraw = Math.max(0, currentMen - floor);

  /** Total allowed across all rows for this mode. */
  ...
  return (
    <Overlay>
      ...
      <Section label="Troops">
        {sourceUnits.map((u) => {
          const key = unitKey(u);
          // Each row is capped by what is left of the shared budget, so the
          // selection can never be pushed past the floor or the free capacity
          // and then blocked only at confirm time.
          const takenElsewhere = takeMen - (counts[key] ?? 0);
          const max = Math.min(u.count, Math.max(0, budget - takenElsewhere));
          return (
            <Row key={key}>
              <span style={{ flex: 1 }}>
