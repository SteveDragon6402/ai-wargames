...
export default function CampaignTable({ campaign }: { campaign: CampaignId }) {
  ...
  return (
    <main className="flex min-h-dvh flex-col items-center justify-center px-6 py-16">
      <a href="/" className="mb-10 text-[13px] text-muted-foreground hover:text-foreground">
        All games
      </a>
      ...
      <div className="w-full max-w-md rounded-sm border border-border bg-card">
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-sm border border-north/30 bg-north-deep/50 px-3 py-2.5">
              ...
              <div className="mt-0.5 text-[12px] text-muted-foreground">Robb Stark, five hosts</div>
            </div>
            <div className="rounded-sm border border-west/30 bg-west-deep/50 px-3 py-2.5">
              ...
              <div className="mt-0.5 text-[12px] text-muted-foreground">Tywin Lannister, two hosts</div>
            </div>
          </div>

          <Button type="button" disabled={loading} onClick={createRoom} className="h-10 w-full text-[13px] font-semibold">
          ...
          <div className="flex gap-2">
            ...
            />
            <Button type="button" variant="outline" disabled={loading || code.length !== 6} onClick={joinRoom} className="h-10 px-5">
              Join
            </Button>
          </div>
        </div>
        ...
        {error && (
          <div className="mx-5 mb-4 rounded-sm border border-bad/40 bg-bad/10 px-3 py-2 text-[13px] text-bad">{error}</div>
        )}

        <div className="border-t border-border px-5 py-3 text-center">
          <a href={`/${campaign}`} className="text-[13px] text-muted-foreground hover:text-foreground">
