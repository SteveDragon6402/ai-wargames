...
| **Siege investment** | Hard + soft | Immediate when a *sole* field faction stands at a garrisonable hold whose `controller` is someone else (or hostile/null) and the garrison still has men — home faction does not matter (e.g. North at Westerlands-held Riverrun). Checked after marches and again after battles/retreats so invest does not wait a turn. Opening day sets `turns = 1` but does **not** decrement food; continued march ticks do. Soft `supplies` update every invest tick. |
...
| **Storm / Sally** | Hard orders → battle | Assault the walls, or sortie (alone or with a relieving field army — one two-front engagement). March and storm are separate orders: arrive/invest this resolve, storm on a later planning turn (you cannot queue move + storm together). After a successful storm, walls are open until the attacker peels men to claim. |
